...
export function getSearchTarget(query: string): string {
  ...
  // Exact SKU/style-code searches should still be normalized so inputs like
  // "hf 4198 001" become "HF4198-001".
  if (isLikelySku(cleaned)) return normalizeSku(cleaned);

  // Keyword searches must preserve word boundaries. Do NOT convert
  // "black cat 4" into "blackcat4" or automatically replace the
  // public store-search phrase with the resolved SKU. The catalog still
  // resolves the SKU for the product panel, but store lookups use the
  // user-visible phrase.
  return cleaned;
}
